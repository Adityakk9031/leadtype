...
| `docsDirName` | Name of the docs subdirectory under `srcDir`. Defaults to `docs`. Set when source docs live in another directory (e.g. `content/docs`). |
| `mounts` | Mounted route paths. Reshapes each doc's `urlPath` so curated starting points, link blocks, and navigation entries match mounted routes. |
